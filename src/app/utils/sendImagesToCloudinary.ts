...
import { v2 as cloudinary } from 'cloudinary';
import { Express } from 'express'; // Import Request type from express
import config from '../config'; // Assuming config file is correctly set up
...
// Function to upload image to Cloudinary
export const sendImageToCloudinary = async (imageName: string, file: Express.Multer.File) => {
  try {
    if (imageName && file) {
      const result = await cloudinary.uploader.upload(file.path, {
        public_id: imageName.trim(), // Ensure no whitespace around public_id
      });

      // Cloudinary automatically handles storage, so no need to delete local file
      return result?.secure_url;
    }
  } catch (error) {
    console.error('Error uploading image to Cloudinary:', error);
